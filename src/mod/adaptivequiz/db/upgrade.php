...
function xmldb_adaptivequiz_upgrade($oldversion) {
    ...
    if ($oldversion < 2023011547) {
        $table = new xmldb_table('adaptivequiz');
        $field = new xmldb_field('browsertimer', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null,0);

        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_mod_savepoint(true, 2023011547, 'adaptivequiz');
    }
    if ($oldversion < 2023011548) {
        $table = new xmldb_table('adaptivequiz');
        $field = new xmldb_field('timeinput', XMLDB_TYPE_TEXT, 'medium', null, null, null, null, null, null);

        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_mod_savepoint(true, 2023011548, 'adaptivequiz');
    }
